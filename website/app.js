...
if(typeof module!=='undefined'&&module.exports) module.exports={localDate,addDays,emptyProgress,applyFeedback,dueWords};
...
let state={view:'today',day:1,query:'',grade:'all',level:'all',cat:'all'};
const comps=[
...
document.querySelectorAll('.nav').forEach(b=>b.onclick=()=>{state.view=b.dataset.view;document.querySelectorAll('.nav').forEach(x=>x.classList.toggle('active',x===b));render()});
...
function drawList(){
 let a=D.vocabulary.filter(x=>(!state.query||x.word.toLowerCase().includes(state.query.toLowerCase()))&&(state.grade==='all'||x.grade===state.grade)&&(state.level==='all'||x.level.startsWith(state.level))&&(state.cat==='all'||x.category===state.cat));
...
function render(){if(state.view==='today')renderToday();else if(state.view==='library')renderLibrary();else if(state.view==='tree')renderTree();else if(state.view==='compare')renderCompare();else renderLesson();}
render();
window.openWord=openWord;window.render=render;window.state=state;
})();
}
