  ...
  nodes: [
    {
      ...
      deep: { logic: 'AT 不强调内部或表面，而强调落在一个被锁定的点上。', scenes: [{ title: '点定位', body: 'at school、at six 和 look at me 都把地点、时间或目标锁定为一个点，而不是强调内部或表面。', example: 'Meet me at the door.' }], structures: '动词 + at + 目标。', chineseTrap: '不要一律翻成“在”；它强调定位点。', studyTip: '先问自己：这里是不是把对象当成一个点？' },
      relations: [
    ...
    },
    {
      ...
      deep: { logic: '空间、时间段和抽象状态都可被理解为有边界的容器。', scenes: [{ title: '边界内部', body: 'in the box、in China 和 in trouble 都把对象、范围或状态理解为有内外边界的容器，表达已在其中。', example: 'The keys are in the bag.' }], structures: 'be + in + 容器/范围。', chineseTrap: 'IN 说的是已在里面，不包含进入动作。', studyTip: '先看画面是“已经在内”还是“正跨进内”。' },
      relations: [
    ...
    },
    {
      ...
      deep: { logic: 'TO 可给出方向和终点；在 TOO...TO... 的 to do 中，to 是不定式标记，整个结构表示程度过高以致后续动作无法实现。', scenes: [{ title: '方向与终点', body: 'go to school、give it to me 和 listen to music 都用 TO 指向目标；too tired to walk 中的 to 则是不定式标记，不表示移动。', example: 'Go to school.' }], structures: '动词 + to + 目标；too + 形容词/副词 + to + 动词。', chineseTrap: 'TO 不等于静态“在”；在 TOO...TO... 中，to 是不定式标记，不是移动介词。', studyTip: '用箭头标出空间方向；遇到 TOO...TO... 时找出因程度过高而无法实现的后续动作。' },
      relations: [
        ...
        { type: 'combination', target: 'too-to', label: 'TOO + adj./adv. + to do → TOO...TO...', explanation: 'TOO 后接形容词或副词，to 是不定式标记；TOO...TO... 表示程度过高以致后续动作无法实现，而不是简单的移动介词。' },
      ],
    },
    {
      ...
      deep: { logic: 'INTO 同时包含方向、边界和进入后的内部落点。', scenes: [{ title: '穿过边界进入', body: 'go into the room 与 put it into the box 表示从外到内；turn into ice 则保留进入另一种状态的转变。', example: 'She walks into the room.' }], structures: '动作动词 + into + 内部目标。', chineseTrap: '不要把 INTO 当作静态 IN；它必须有进入或转变。', studyTip: '看是否发生“外 → 穿界 → 内”的三步画面。' },
      relations: [
    ...
    },
    {
      ...
      deep: { logic: '-ING 让动作具有过程感，常需 BE 来标出主体正处于这个过程。', scenes: [{ title: '展开的动作过程', body: 'is running 和 are studying 用 BE 标出主体正在经历的过程；keep moving 也把动作看作持续展开。', example: 'They are studying.' }], structures: 'be + 动词-ing。', chineseTrap: '不要机械翻译为“正在”；先理解动作是否被当成过程。', studyTip: '想象动作有连续帧，而不是一个瞬间。' },
      relations: [
    ...
    },
    {
      ...
      deep: { logic: 'THE 不是给名词贴中文标签，而是把注意力锁到可识别的特定对象。', scenes: [{ title: '可定位的对象', body: 'the door、the teacher 和 the book on the desk 都要求听者能凭共享语境或补充信息定位到同一个对象。', example: 'Close the door.' }], structures: 'the + 已知或可定位的名词。', chineseTrap: '中文常不说“那个”，英语仍可能需要 THE。', studyTip: '问自己：听者能否凭上下文找到同一个对象？' },
      relations: [
    ...
    },
    {
      ...
      deep: { logic: 'IF 不保证事情发生，它只规定“在这个条件下，后面的话成立”。', scenes: [{ title: '条件空间', body: 'if it rains、if you need help 和 if I have time 都先提出尚未确定的条件，再讨论该条件下可能成立的结果。', example: 'If it rains, we stay home.' }], structures: 'if + 条件，结果；结果 if + 条件。', chineseTrap: '不要把 IF 当成必然因果；它表达的是条件。', studyTip: '先圈出条件，再找该条件下的结果。' },
      relations: [
    ...
    },
    {
      ...
      deep: { logic: 'TOO 提供超出界限的程度，to 是不定式标记；整个结构表示程度过高以致后续动作无法实现。', scenes: [{ title: '过度导致不能做', body: 'too tired to walk、too small to read 和 too late to call 都先给出超过界限的程度，再给出因此无法实现的后续动作。', example: 'It is too late to call.' }], structures: 'too + 形容词/副词 + to + 动词。', chineseTrap: '它常带负向结果，不等于单纯“很”；这里的 to 是不定式标记。', studyTip: '先找“超出什么界限”，再找因程度过高而无法实现的后续动作。' },
      relations: [
        { type: 'system', target: 'state-action', label: '所属：状态与动作', explanation: 'TOO...TO... 在状态与动作系统中表达程度造成的动作结果。' },
        { type: 'combination', target: 'to', label: 'TOO + adj./adv. + to do → TOO...TO...', explanation: 'TOO 后接形容词或副词，此处的 to 是不定式标记；TOO...TO... 表示程度过高以致后续动作无法实现，不是简单的移动介词。' },
    ...
    },
    {
      ...
      deep: { logic: 'SEE 重结果，不要求主动把目光投过去。', scenes: [{ title: '已经获得的结果', body: 'see a bird 和 see the problem 都强调对象已经进入视觉或理解范围；see you tomorrow 表示之后见到的结果。', example: 'I see a bird.' }], structures: 'see + 对象；can see + 对象。', chineseTrap: 'SEE 不是主动盯着看，而是已经获得视觉结果。', studyTip: '问“看到了吗”，答案相关时优先想 SEE。' },
      relations: [
    ...
    },
    {
      ...
      deep: { logic: 'LOOK 是方向动作，因此常需 AT、UP、DOWN 等词补出方向或目标。', scenes: [{ title: '把目光投向目标', body: 'look at me、look up 和 look around 都需要补出目光投向的目标或方向，不能保证已经看见。', example: 'Look at me.' }], structures: 'look + 方向；look at + 目标。', chineseTrap: 'LOOK 不保证看见了；它只说明目光投过去。', studyTip: '画出眼睛到目标的箭头，补全它朝哪里。' },
      relations: [
    ...
    },
    {
      ...
      deep: { logic: 'WATCH 需要时间长度，适合节目、比赛、孩子和风险等会变化的对象。', scenes: [{ title: '持续跟随变化', body: 'watch TV、watch a game 和 watch the baby 都把注意力持续放在会变化的对象或过程上，而不是一眼看见。', example: 'Watch the baby.' }], structures: 'watch + 过程性对象。', chineseTrap: 'WATCH 不是所有“看”的通用替换；它需要持续过程。', studyTip: '若对象会变化，问自己是否需要持续跟随。' },
      relations: [
